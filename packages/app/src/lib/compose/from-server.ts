...
export const COMPOSE_STEPS = [
	{
		...
		detail:
			"the graph · your book from the aqua subgraph · pair data still stubbed",
	},
	{
	...
	},
	{
		label: "Validating",
		detail:
			"deterministic gate over the signed output — rejects, never rewrites",
	},
] as const;
...
type ServerStrategy =
	ServerComposeResult["recommendation"]["strategies"][number];
...
function toUiStrategy(s: ServerStrategy): UiStrategy {
	...
	const feeText = Number.isFinite(feeBps)
		? `${pct(feeBps)} maker fee`
		: "no maker fee";
	...
	return {
		...
		facts: [
			...
			...(Number.isFinite(feeBps)
				? [{ label: "FEE", value: `${pct(feeBps)} maker fee` }]
				: []),
		...
		slots: [
			...
			},
			{
				...
				params: s.slots.band
					? `${band} around mid at observation`
					: "full range",
			},
			{
				index: 3,
