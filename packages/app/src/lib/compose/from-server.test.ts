...
test("fromServer maps a recommendation into the UI shapes", () => {
	...
	for (const n of ["curve", "band", "fee", "deadline"])
...
test("fromServer re-parses shipInputs amounts from decimal strings into bigints", () => {
	...
	assert.equal(
		shipInput.strategyHash,
		SERVER_RESULT.shipInputs[0].strategyHash,
	);
	...
	assert.equal(
		shipInput.amounts[0],
		BigInt(SERVER_RESULT.shipInputs[0].amounts[0]),
	);
	assert.equal(
		shipInput.amounts[1],
		BigInt(SERVER_RESULT.shipInputs[0].amounts[1]),
	);
