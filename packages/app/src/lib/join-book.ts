 ...
 * React, no clock — a plain function of its inputs, unit-testable without a
 * chain or a subgraph.
 *
 * `strategyHash` is the join key. The chain row is authoritative for everything
 * it can prove: legs (ceiling from the ship-funding event, consumed from
 * balance movement), fills, deadline and slot rows (decoded from the program
 * bytes), docked state. The cache supplies only what a recommendation knows and
 * the chain does not: ship-time wording, band terms, risk rating, provenance. A
 * cache-miss row is still a full position — just labelled with what was decoded.
 */
...
export function toPosition(
  ...
  return {
    ...p,
    // Recommendation-only fields: the cache's wording and terms are richer than
    // a program decode can recover (exact band %, risk, who produced it) — but
    // legs, fills, dockedAt stay the chain's.
    pair: cached.pair,
    ...
    // Decoded slot rows exist even for foreign programs; prefer the cached ones
    // (band % instead of raw deltas) when the cache has them.
    slots: cached.slots.length > 0 ? cached.slots : p.slots,
    // The decode is authoritative (it read the shipped bytes) — the cache only
    // fills in when the bytes would not decode at all.
    deadline: p.deadline ?? cached.deadline,
  };
}

/** A cache entry rebuilt as a fresh Position: nothing consumed, no fills. The optimistic entry shown right after a ship, before the subgraph indexes it. */
...
/** Inverse of `positionFromMeta` — what to cache from an already-built Position (a demo fixture, say). Drops `consumed`/`fills`/`dockedAt`: those are the chain's to report. */
...
/** "WETH / USDC" — sorted by decimals descending, matching the fixtures. */
